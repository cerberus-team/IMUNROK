...
namespace IMUNROK.Common
{
    ...
    public static class HeldRig
    {
        ...
        public static bool TryGet(string toolId, out Pose pose)
        {
            switch (toolId)
            {
                // 등불 — <b>장대 끝에 매달아 앞으로 내어 든다.</b>
                //
                // 여태 0.55m 앞 허리께에 두었더니 화면을 통째로 먹었다. 재 보면
                // 오른쪽으로 <b>30% 비어져 나가고</b> 아래로 <b>150% 잠겨</b>, 셋 중
                // 하나쯤만 화면에 남았다 — 든 물건이 아니라 화면을 가린 판이었다.
                //
                // <b>작게 만들어 고치지 않는다.</b> 같은 등불이 관아에 놓여 있고
                // (Onggojip_Gwana 의 「놓인등불」, 21배) 집어 들었더니 작아지면
                // 든 것이 아니라 바뀐 것이 된다. 크기는 그대로 두고 <b>멀리 든다</b> —
                // 애초에 이 물건은 장대에 매달린 등롱이고, 장대는 멀리 들라고 있다.
                // 1.23m 는 장대 길이만큼이다.
                //
                // y 를 30도 돌리는 까닭: 장대의 손잡이 끝이 <b>화면 오른쪽 아래로
                // 빠져나가야</b> 그 너머에 손이 있는 것으로 읽힌다. 안 돌리면 장대가
                // 화면을 가로질러 허공에서 끊긴다.
                case "lantern":
                    pose = new Pose { position = new Vector3(0.54f, -0.195f, 1.23f), euler = new Vector3(0f, 30f, 0f), scale = 19.5f };
                    return true;
