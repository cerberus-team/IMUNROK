...
namespace IMUNROK.Common
{
    ...
    ///
    /// 대사는 SubtitleView 로 띄운다.
    /// 카메라를 뺏지 않는다 — 플레이어는 계속 1인칭 조종이다. 대사만 화면에 흐르고
    ...
    public class GateKnockSequence : MonoBehaviour
    {
        ...
        private static Line[] DefaultLines() => new[]
        {
            ...
            // ⚠️ 이름패에 <b>(甲)</b> 을 달아 두었었다. 우리끼리 갑·을로 갈라 부르는 표인데
            //    그것이 그대로 화면에 나갔다 — 손님은 아직 이 사람이 진짜인지 가짜인지
            //    모르는데, 이름패가 <b>「둘 중 하나」임을 먼저 알려 준다</b>. 사건이
            //    시작도 하기 전에 반전이 새는 자리다. 화면에는 「옹덕구」만 나간다.
            new Line { speaker = "옹덕구", text = "허허, 누추하나 드시오. 사랑에 자리를 봐드리리다." },
        };
    }
}
