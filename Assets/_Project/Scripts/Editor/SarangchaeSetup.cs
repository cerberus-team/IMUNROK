...
namespace IMUNROK.Common.EditorTools
{
    ...
    public static class SarangchaeSetup
    {
        ...
        ///
        /// 물건을 가리킬 때마다 두 문장씩 떠오르면 읽다가 조사가 끊긴다. 좁은 화면에서는
        ...
        private static void ShortenTexts()
        {
            ...
            // 서랍에는 <b>사삿집 문서 두 장</b>만 둔다. 관을 속인 자취(호적대장·호구단자)는
            // 한 서랍에 같이 넣지 않는다 — 넉 장이 한 자리에서 다 나오면 뒤진 것이 아니라
            // 받아 든 것이 된다. 그 둘은 장롱 안과 병풍 뒤에 따로 숨겨 두었다.
            SetWords("문갑_서랍",
                     "문서를 넣어 두는 궤다.",
                     "문서 두 장이 개켜져 있다.",
                     "(눌러 잡고 서랍 빼기)", null,
                     "문갑 서랍에 사삿집 문서 *두 장* — 물목 장부와 차용증.");   // 서랍 자체는 단서가 아니다

            // 장롱은 여기서 손댈 것이 없다 — 문(DoorController)이 이미 달려 있고,
            // 그 문을 열면 개켜 둔 이불 사이로 호구단자 자락이 나온다.
            SetWords("병풍_젖히기",
                     "산수를 그린 여섯 폭 병풍이 서벽을 가리고 서 있다.",
                     "밀어 두니 가려 있던 벽 밑이 드러난다 — 바닥에 *종이 한 장*.",
                     "(눌러 잡고 옆으로 밀기)", null, null);   // 젖히면 보료를 뚫는다. 미는 것이 맞다
