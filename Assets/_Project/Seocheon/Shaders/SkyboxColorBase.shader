Shader "Seocheon/SkyboxColorBase"
{
    Properties
    {
        ...
        [NoScaleOffset] _MainTex ("Equirectangular Colour Panorama", 2D) = "grey" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _MainTex;
            half4 _Tint; half _Exposure; float _Rotation; half _ColorBlurMip;
            half _DramaGain; half4 _ColorBalance; half _Saturation; half _Contrast;
            half _HorizonLift; half4 _ZenithColor; half4 _NadirColor;
            half _PoleBlendStart; half _PoleBlendEnd;
            ...
            v2f vert(appdata_t v) { v2f o; UNITY_SETUP_INSTANCE_ID(v); UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(o); o.vertex=UnityObjectToClipPos(v.vertex); o.dir=v.vertex.xyz; return o; }
            half4 frag(v2f i) : SV_Target
            {
                float3 d=normalize(i.dir);
                float compass=atan2(d.x,d.z)+radians(_Rotation);
                float2 uv=float2(frac(compass/(2.0*UNITY_PI)+0.5),saturate(asin(clamp(d.y,-1.0,1.0))/UNITY_PI+0.5));
                // High mip retains the approved S33 palette while removing its painted cloud silhouettes.
                ...
                half lum=dot(c,half3(.2126,.7152,.0722));
                c=lerp(lum.xxx,c,_Saturation);
                c=(c-.5h)*_Contrast+.5h;
                half horizon=1.0h-saturate(abs(d.y)*6.0h);
                c+=_HorizonLift*horizon*half3(1.0h,.78h,.88h);
                half pole=smoothstep(_PoleBlendStart,_PoleBlendEnd,abs(d.y));
                c=lerp(c,d.y>=0.0?_ZenithColor.rgb:_NadirColor.rgb,pole);
