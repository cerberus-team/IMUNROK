// ─────────────────────────────────────────────────────────────────────────────
//  ★견우 꾸러미 IMUNROK.Ui.DialogueUI (1525줄) 의 사본이다. 2026-08-26 에 떠 왔다.
//
//  ■ 왜 복사했나
//    서천에는 <선택지 4개>가 있어야 한다 — VR 에서는 글쇠를 칠 수 없어 이것이 기본 입력이다.
//    견우 대화창에는 그 줄이 없고, 판 높이(SeocheonBarStyle.TotalHeight)가 여백의 합으로
//    딱 맞게 계산돼 있어 밖에서 끼워 넣으면 배치가 어긋난다.
//    꾸러미 파일을 고치면 다음 배포본에 덮여 사라지므로(문서: '언제든 다시 구워진다'),
//    사본을 우리 폴더에 두고 여기서만 고친다.
//
//  ■ 무엇이 원본과 갈리나 — <배치 수치뿐>이다
//    · 색   : IMUNROK.Ui.DialogueUI.Palette() 를 <그대로 호출>한다 → 팀원이 바꾸면 따라간다
//    · 글꼴 : IMUNROK.Ui.UiSkin.Font (조선 궁서체) 를 <그대로 쓴다>  → 마찬가지
//    · 치수 : SeocheonBarStyle 로 갈라졌다 (선택지 줄이 하나 더 있다)
//    그래서 '한 게임으로 보이게 한다'는 팀 규약은 지켜진다.
//
//  ■ 되돌리는 조건
//    팀원이 다음 꾸러미에서 하단바에 선택지 줄을 옵션으로 넣어 주면 <이 파일은 지운다>.
//    그때까지의 임시본이다.
// ─────────────────────────────────────────────────────────────────────────────
...
namespace IMUNROK.Seocheon
{
    ...
    /// <summary>
    /// 하단 바의 치수 — 견우 <c>IMUNROK.Ui.BottomStyle</c> 에 <b>선택지 줄</b>을 더한 것.
    ///
    /// 원본의 요점을 그대로 지킨다: <b>판 높이를 여백의 합으로 계산</b>한다
    /// (<see cref="TotalHeight"/>). 그래서 항목을 더하면 판이 <b>딱 그만큼만</b> 커지고
    /// 나머지 배치는 저절로 맞는다 — 선택지 줄을 끼워 넣을 수 있는 이유가 이것이다.
    /// </summary>
    public struct SeocheonBarStyle
    {
        ...
        public float lineToInput;   // 대사(또는 선택지) ↔ 글쇠 칸
        public float inputToFoot;   // 글쇠 칸 ↔ 아래 조작 안내
        public float footToEdge;    // 조작 안내 ↔ 아래 테두리
        public float footH;         // 조작 안내 줄 높이

        // ── ★서천 전용 — 선택지 (2026-08-26 자리만 잡음 → 08-27 채움) ────────
        //   VR 에서는 글쇠를 칠 수 없다. 선택지가 사실상 유일한 입력이라
        //   대사와 입력줄 <b>사이</b>에 앉힌다.
        //   ⚠️ 칸 나눔이 모드마다 다르다 — <b>폭이 다르기 때문</b>이다.
        //      PC 2900 에서 1×4 면 칸당 약 690 으로 한 줄에 든다.
        //      VR 1500 에서 1×4 면 칸당 약 350 이라 글자가 접힌다 → 2×2 로 눕힌다.
        public float lineToAsk;     // 대사 ↔ 선택지
        public float askH;          // 선택지 한 칸 높이
        public float askGap;        // 선택지 칸 사이
        public int askCols;         // 가로 칸 수
        public int askRows;         // 세로 줄 수. ★0 이면 선택지 줄이 아예 없다(지난 시안 E·F)

        /// <summary>
        /// 안내줄을 <b>이름패 줄 오른쪽</b>으로 올릴지 (2026-08-27).
        ///
        /// ★판 높이는 「고치지 말 것」이다. 선택지 줄을 넣을 자리를 어디선가 덜어 와야 하는데,
        ///   PC 는 머리 띠가 2720 단위나 비어 있다 — 안내를 거기 얹으면 아래에서 한 줄
        ///   (inputToFoot + footH) 이 통째로 빈다. VR 은 머리 띠가 1380 뿐이고 안내 글자가
        ///   34 단위라 <b>들어가지 않는다</b> — 그래서 VR 은 안내를 아래에 그대로 둔다.
        /// </summary>
        public bool footInNameRow;

        /// <summary>선택지 덩어리 전체 높이. 줄이 없으면 0 이다.</summary>
        public float AskBlockH
        {
            get { return askRows <= 0 ? 0f : askH * askRows + askGap * (askRows - 1); }
        }

        /// <summary>이름패 높이 — 글자에 위아래 여유를 붙인 것. 닫기 ×도 같은 높이.</summary>
        public float NameH { get { return nameSize + 18f; } }

        /// <summary>
        /// 여백까지 더한 판 높이. 머리 → 대사 → <b>선택지</b> → 입력줄 → 안내 순의 합이다.
        ///
        /// ★선택지 줄(askRows=0)과 안내 자리(footInNameRow)를 빼면 <b>견우 원본과 글자 그대로 같은 식</b>이다 —
        ///   지난 시안 E·F 는 원본과 한 단위도 달라지지 않는다.
        /// </summary>
        public float TotalHeight
        {
            get
            {
                return padTop + NameH + nameToRule + ruleH + ruleToLine
                     + lineBoxH
                     + (askRows > 0 ? lineToAsk + AskBlockH : 0f)
                     + lineToInput + inputH
                     + (footInNameRow ? 0f : inputToFoot + footH)
                     + footToEdge;
            }
        }
    }

    public class SeocheonDialogueBar : MonoBehaviour
    {
        ...
        static Geom GeomOf(DialogueLayout l)
        {
            switch (l)
            {
                ...
                //   높이는 <see cref="SeocheonBarStyle.TotalHeight"/> 가 여백까지 더해 계산한다 —
        ...
        static SeocheonBarStyle StyleOf(DialogueLayout l)
        {
            switch (l)
            {
                // 대사 자리 높이 = 글자크기 × 1.25(궁서체 줄높이) × 줄 수 + 여유 6
                case DialogueLayout.E_하단바_얇게:   // 2줄
                    return new SeocheonBarStyle { lineBoxH = LineBox(38, 2), lineSize = 38,
                ...
                case DialogueLayout.F_하단바_여유:   // 4줄
                    return new SeocheonBarStyle { lineBoxH = LineBox(46, 4), lineSize = 46,
                ...
                case DialogueLayout.하단바_확정 when UiModes.IsVr:
                    ...
                    //
                    //   ── ★서천 선택지 줄을 넣으며 다시 잰 자리 (2026-08-27) ──────────
                    //   ⚠️ <b>VR 은 대사가 3줄 → 2줄로 줄었다.</b> 판 높이 535 는 「고치지 말 것」인데,
                    //      2×2 선택지(114) 가 들어가려면 그만한 자리를 어디선가 덜어 와야 한다.
                    //      후보를 다 재어 보았다:
                    //        · 안내줄을 머리 띠로 올리기 → ★안 된다. VR 안내는 34단위 글자가 40자쯤이라
                    //          1360 단위인데 머리 띠에 남는 폭이 그만큼 없다 (PC 는 된다 — 아래 참고).
                    //        · 입력줄 빼기 → ★안 된다. 목소리로 받아 적은 글이 그 칸에 뜬다.
                    //          칸을 없애면 <b>무엇으로 전해지는지 못 보고</b> 던지게 된다 (팀 주석의 사정 그대로).
                    //        · 대사 3줄 → 2줄 → ★이것을 골랐다. 굴려 읽기(RectMask2D)가 이미 있어
                    //          긴 대꾸도 잘리지 않고, VR 에서 한 줄에 약 36자가 들어가 2줄이면 72자다.
                    //      합: 18+54+12+3+20 +147 +16 +114 +16 +66 +16+36 +17 = <b>535</b>
                    return new SeocheonBarStyle { lineBoxH = LineBox(46, 2), lineSize = 46,
                                             nameSize = 36, inputSize = 34, footSize = 34, inputH = 66f,
                                             padX = 60f, padTop = 18f, nameToRule = 12f, ruleH = 3f,
                                             ruleToLine = 20f, lineToInput = 16f,
                                             inputToFoot = 16f, footToEdge = 17f, footH = 36f,
                                             lineToAsk = 16f, askH = 52f, askGap = 10f,
                                             askCols = 2, askRows = 2, footInNameRow = false };
                ...
                default:                             // D — 보통, 3줄 · 확정안 (PC)
                    ...
                    //
                    //   ── ★서천 선택지 줄 (2026-08-27) ────────────────────────────────
                    //   PC 는 <b>대사 3줄을 지켰다.</b> 머리 띠 안쪽이 2720 단위인데 이름패·녹음
                    //   표시를 다 놓고도 900 넘게 남아, <b>안내줄을 그 오른쪽으로 올렸다</b>.
                    //   그러면 아래에서 한 줄(inputToFoot 22 + footH 24 = 46)이 통째로 비고,
                    //   입력줄을 62→46 으로 눌러 16 을 더 벌어 1×4 선택지(62)가 들어간다.
                    //     옛 아래쪽: lineToInput 34 + inputH 62 + 22 + 24 + 19 = 161
                    //     새 아래쪽: lineToAsk 16 + askH 62 + 14 + inputH 46 + 23 = 161  ← ★같다
                    //   합: 14+50+10+3+24 +199 +16 +62 +14 +46 +23 = <b>461</b>
                    return new SeocheonBarStyle { lineBoxH = LineBox(42, 3), lineSize = 42,
                                             nameSize = 32, inputSize = 28, footSize = 19, inputH = 46f,
                                             padX = 90f, padTop = 14f, nameToRule = 10f, ruleH = 3f,
                                             ruleToLine = 24f, lineToInput = 14f,
                                             inputToFoot = 0f, footToEdge = 23f, footH = 0f,
                                             lineToAsk = 16f, askH = 62f, askGap = 12f,
                                             askCols = 4, askRows = 1, footInNameRow = true };
            }
        }
        ...
        // ★색은 정의하지 않는다 — 견우 IMUNROK.Ui.DialogueUI.Palette() 를 그대로 부른다.
        //   팀원이 먹빛·투명도를 바꾸면 서천도 저절로 따라간다 (팀 규약: 색은 통일).
        ...
        public static SeocheonDialogueBar Instance { get; private set; }
        ...
        // ── ★서천 — 선택지 4개 ────────────────────────────────────────────
        //   판 <b>안</b>에 넣는다. 밖에 띠로 얹던 예전 안(SeocheonDialogueExtras)은
        //   판이 다시 지어질 때마다 다시 붙여야 했고, 판과 따로 놀아 한 벌로 안 보였다.
        readonly DialogueHotspot[] askOptSpots = new DialogueHotspot[4];
        readonly TextMeshProUGUI[] askOptLabels = new TextMeshProUGUI[4];

        /// <summary>서천이 더 얹은 것(선택지·어절)을 아는 말 상대. 없으면 견우 화면 그대로 돈다.</summary>
        ISeocheonBarSource extras;

        // ── ★서천 — 어절 지목 ─────────────────────────────────────────────
        //   판정은 <see cref="WordRegionBuilder"/> 가 한다 — 수첩·카드 화면과 <b>같은 구현</b>이다.
        //   표시는 대사(lineRect)의 자식이라 굴려 읽어도 글자와 함께 움직인다.
        readonly List<WordRegion> regions = new List<WordRegion>();
        readonly List<Image> wordHoverPool = new List<Image>();
        readonly List<Image> wordMarkPool = new List<Image>();
        RectTransform pickRoot;
        WordPickNote.Sentence pickSource;
        int hoverWord = -1;

        /// <summary>가리킨 어절. ★유효/오답을 가르지 않는다 — 지목 시점에는 판정하지 않는다.</summary>
        static readonly Color WordHoverColor = new Color(1f, 0.85f, 0.4f, 0.28f);
        /// <summary>지목을 마친 어절의 밑줄. ★위와 같은 이유로 한 가지뿐이다.</summary>
        static readonly Color WordMarkColor = new Color(1f, 1f, 1f, 0.35f);
        ...
        // ─────────────────────────────────────────────────────────
        public static SeocheonDialogueBar Ensure()
        {
            if (Instance != null) return Instance;
            var go = new GameObject("대화_판");
            DontDestroyOnLoad(go);
            Instance = go.AddComponent<SeocheonDialogueBar>();
            go.SetActive(false);
            return Instance;
        }
        ...
        public void Open(IDialogueSpeaker npc, IDialogueBackend s)
        {
            owner = npc;
            session = s;
            // ★서천이 더 얹은 것(선택지·어절 후보)을 아는 상대인지 본다. 아니면 견우 화면 그대로다.
            extras = s as ISeocheonBarSource;
        ...
        void LateUpdate()
        {
            ...
            //   PC 2900×461 과 VR 1500×535 는 치수·글자·칸 나눔이 아예 달라 다시 짓는 수밖에 없다.
            if (builtForVr != UiModes.IsVr)
            {
                Rebuild(Layout);
                ApplyPose();
                ApplyInputRowMode();
                if (hintText != null) hintText.text = HintLine;
                Refresh();
                ActivateField();
                return;
            }
            ...
            // ★서천 — 가리키개를 몬다. HandleVoice 가 Hovered 를 보므로 <b>그보다 먼저</b> 돌아야 한다.
            DrivePointer();
        ...
        void Refresh()
        {
            ...
            if (lineText != null && line != shownLine)
            {
                ...
                ApplyLineScroll();
                RebuildWordRegions();     // ★서천 — 글이 바뀌면 어절 자리도 바뀐다
            }
            ...
            // ★증거 제시 — 서천에는 내밀 것이 없다(Presentables 가 늘 빈 목록이다).
            //   단추를 감추지 않고 <b>흐리게 둔다</b>: 자리가 사라지면 판 배치가 모드마다 달라지고,
            //   나중에 소지품이 붙었을 때 자리를 다시 잡아야 한다.
            if (presentSpot != null)
            {
                var items = session.Presentables();
                presentSpot.interactable = idle && items != null && items.Count > 0;
                presentSpot.SetHovered(false);
            }

            // ★선택지 — 말 상대가 준 물음을 칸에 올린다. 빈 칸은 감춘다.
            var asks = extras != null ? extras.Asks : null;
            for (int i = 0; i < askOptSpots.Length; i++)
            {
                if (askOptSpots[i] == null) continue;
                bool has = asks != null && i < asks.Count && asks[i] != null
                        && !string.IsNullOrEmpty(asks[i].label);
                if (askOptSpots[i].gameObject.activeSelf != has) askOptSpots[i].gameObject.SetActive(has);
                if (!has) continue;
                askOptSpots[i].interactable = idle;
                askOptSpots[i].SetHovered(false);
                if (askOptLabels[i] != null) askOptLabels[i].text = asks[i].label;
            }
        }
        ...
        /// <summary>
        /// ★서천 — 선택지를 골랐다. <b>자유 입력과 같은 길</b>로 흘러간다
        /// (말 상대가 <c>Ask(문구)</c> 를 부른다) — 화면은 어느 쪽으로 왔는지 모른다.
        /// </summary>
        void ChooseAsk(int index)
        {
            if (extras == null || session == null || session.Busy) return;
            if (voice.Recording || voiceBusy) return;
            if (field != null) field.text = string.Empty;
            extras.ChooseAsk(index);
            ActivateField();
        }
        ...
        void RequestExit()
        {
            ...
            if (rig != null) { rig.ExitFocus(); return; }

            // ★서천 — 리그를 쓰지 않는다(대화는 눈앞 당기기·비네트가 필요 없다).
            //   리그가 없으면 그냥 닫는다. 안 그러면 ×와 Esc 가 <b>아무 일도 안 한다</b>.
            Close();
        }
        ...
        public void PointAt(Ray ray)
        {
            ...
            if (micSpot != null) micSpot.SetHovered(micSpot == Hovered);
            for (int i = 0; i < askOptSpots.Length; i++)
                if (askOptSpots[i] != null) askOptSpots[i].SetHovered(askOptSpots[i] == Hovered);

            // ★서천 — 단추 위가 아닐 때만 어절을 짚는다 (단추가 늘 이긴다).
            HoverWord(Hovered == null ? ray : (Ray?)null);
        }
        ...
        public void ClickHovered()
        {
            ...
            // ★서천 — 단추 위가 아니면 어절을 짚은 것이다.
            if (Hovered == null) { TryPickWord(); return; }
            if (!Hovered.interactable) return;

            // ★서천 — 선택지는 손잡이 종류가 「묻기」와 같다(꾸러미 enum 을 못 늘린다).
            //   그래서 <b>어느 칸이 눌렸는지를 먼저</b> 가려낸다.
            int opt = System.Array.IndexOf(askOptSpots, Hovered);
            if (opt >= 0) { ChooseAsk(opt); return; }
        ...
        void ApplyLineScroll()
        {
            ...
            // ★서천 — 어절 표시도 글자와 <b>똑같이</b> 굴러야 한다 (형제라 저절로 따라오지 않는다).
            if (pickRoot != null) pickRoot.anchoredPosition = lineRect.anchoredPosition;
        ...
        //  ★서천 — 어절 지목
        //
        //  ■ 왜 꾸러미를 안 고쳐도 되는가 (2026-08-27 실측으로 확인한 것)
        //    · <see cref="WordRegionBuilder.Build"/> 가 <b>아무 TMP 나 받는 public static</b> 이다.
        //      대사 라벨을 그대로 넘기면 된다 — 전용 부품(DialogueLineView)을 거칠 필요가 없다.
        //    · 판정은 <b>대사 RectTransform 기준</b>이라 굴려 읽어도 저절로 맞는다.
        //    · 대사 TMP 는 <c>raycastTarget=false</c> 다 — UGUI 이벤트가 아니라
        //      <b>좌표 계산</b>으로 짚는다. 서천이 이미 쓰던 방식과 같다.
        //
        //  ■ ★유효/오답이 눈에 띄면 안 된다
        //    가리킴 색도, 지목 밑줄도 <b>한 가지뿐</b>이다. 판정은 나중에 결합할 때만 일어난다.
        // ─────────────────────────────────────────────────────

        /// <summary>새 대꾸가 왔다 — 어절 자리를 다시 뜬다.</summary>
        void RebuildWordRegions()
        {
            regions.Clear();
            ClearPool(wordHoverPool);
            ClearPool(wordMarkPool);
            hoverWord = -1;

            pickSource = extras != null ? extras.PickSource : null;
            if (lineText == null || pickSource == null || string.IsNullOrEmpty(pickSource.text)) return;

            // ⚠️ 화면에 뜬 글과 짚을 글이 <b>같은 문자열이어야</b> 글자 번호가 맞는다.
            //    (말 상대가 문장을 이을 때 화면과 같은 방식으로 잇는지 여기서 한 번 더 본다.)
            if (pickSource.text != lineText.text) return;

            lineText.ForceMeshUpdate();     // ★Build 전에 메시가 짜여 있어야 한다
            WordRegionBuilder.Build(lineText, pickSource, 3f, 2f, regions);
        }

        /// <summary>가리킨 자리가 어느 어절인지. 광선이 없으면(단추 위였다면) 강조를 지운다.</summary>
        void HoverWord(Ray? ray)
        {
            if (!ray.HasValue || regions.Count == 0 || lineText == null || lineViewport == null)
            { SetWordHover(-1); return; }

            var plane = new Plane(-transform.forward, transform.position);
            float d;
            if (!plane.Raycast(ray.Value, out d)) { SetWordHover(-1); return; }
            Vector3 world = ray.Value.GetPoint(d);

            // ★틀 밖으로 굴려 올라간 글은 <b>안 보인다</b> — 안 보이는 어절이 짚히면 안 된다.
            Vector2 inFrame = lineViewport.InverseTransformPoint(world);
            if (!lineViewport.rect.Contains(inFrame)) { SetWordHover(-1); return; }

            Vector2 local = lineRect.InverseTransformPoint(world);
            SetWordHover(WordRegionBuilder.Find(regions, local));
        }

        void SetWordHover(int index)
        {
            if (hoverWord == index) return;
            hoverWord = index;
            ClearPool(wordHoverPool);
            if (index < 0 || index >= regions.Count) return;
            DrawWordRects(regions[index].rects, wordHoverPool, WordHoverColor, false);
        }

        /// <summary>지금 가리킨 어절을 지목한다. 짚을 것이 없었으면 false.</summary>
        bool TryPickWord()
        {
            if (hoverWord < 0 || hoverWord >= regions.Count) return false;
            WordRegion reg = regions[hoverWord];
            if (reg.consumed) return false;

            reg.consumed = true;
            SetWordHover(-1);
            DrawWordRects(reg.rects, wordMarkPool, WordMarkColor, true);   // 밑줄은 덧쌓인다

            // ★수첩에 적는 일은 말 상대가 한다 — 화면이 단서 코드표를 알면 안 된다.
            if (extras != null) extras.NotifyWordPicked(reg.option);
            return true;
        }

        /// <param name="underline">참이면 얇은 밑줄, 거짓이면 어절을 덮는 옅은 상자.</param>
        void DrawWordRects(List<Rect> rects, List<Image> pool, Color color, bool underline)
        {
            if (pickRoot == null || lineRect == null) return;
            int at = underline ? pool.Count : 0;          // 밑줄은 쌓고, 가리킴은 처음부터 다시 쓴다
            for (int i = 0; i < rects.Count; i++)
            {
                Image img = WordMarkImage(pool, at + i);
                RectTransform rt = img.rectTransform;
                rt.pivot = Vector2.zero;
                Rect r = rects[i];
                rt.sizeDelta = underline ? new Vector2(r.width, 2f) : new Vector2(r.width, r.height);
                rt.position = lineRect.TransformPoint(new Vector3(r.x, underline ? r.y + 2f : r.y, 0f));
                img.color = color;
                img.gameObject.SetActive(true);
            }
        }

        Image WordMarkImage(List<Image> pool, int index)
        {
        ...
        static void ClearPool(List<Image> pool)
        {
        ...
        /// ★서천 — 가리키개를 <b>스스로 몬다</b>.
        ///
        /// 견우 쪽은 <see cref="DebugFocusRig"/> 가 <see cref="FocusInteractable"/> 을 거쳐
        /// <see cref="PointAt"/>·<see cref="ClickHovered"/>·<see cref="Scroll"/> 을 몰아 준다.
        /// 서천 NPC 는 포커스 리그를 쓰지 않으므로(눈앞 당기기·비네트가 필요 없다)
        /// <b>아무도 몰아 주지 않아 단추가 안 눌린다</b>. 그래서 여기서 직접 읽는다.
        /// 리그가 있는 씬에서 두 번 불려도 같은 값이라 무해하다.
        /// </summary>
        void DrivePointer()
        {
            var ptr = UiPointers.Get(eye);
            if (ptr == null || !ptr.Available) return;

            PointAt(ptr.PointRay);
            if (ptr.BackDown) { RequestExit(); return; }
            if (ptr.PressDown) ClickHovered();
            float sc = ptr.ScrollRaw;
            if (Mathf.Abs(sc) > 0.01f) Scroll(sc);
        }
        ...
        void Rebuild(DialogueLayout layout)
        {
            ...
            // ★서천 — 판을 비우면 우리가 얹은 것도 같이 사라진다. 손잡이를 놓아 준다.
            for (int i = 0; i < askOptSpots.Length; i++) { askOptSpots[i] = null; askOptLabels[i] = null; }
            regions.Clear();
            wordHoverPool.Clear();
            wordMarkPool.Clear();
            pickRoot = null;
        ...
        void BuildBottom()
        {
            ...
            float headRight = hw - st.padX - nameH - 20f;   // 닫기 × 왼쪽까지

            // ★서천 — 안내줄을 머리 띠 <b>오른쪽 끝</b>에 올린다 (PC 만. 까닭은 StyleOf 주석).
            //   ⚠️ 오른쪽 맞춤 글은 rect 의 오른쪽 끝에 붙는다 — 중심을 폭의 절반만큼 당겨 온다
            //      (「더 있음」에서 겪은 것과 같은 함정이다).
            float headHintW = 0f;
            if (st.footInNameRow)
            {
                headHintW = 980f;
                hintText = MakeText(root, "안내", st.footSize, TextAnchor.MiddleRight, pal.dim);
                Place(hintText.rectTransform,
                      new Vector2(headRight - headHintW * 0.5f, headY), new Vector2(headHintW, 30f));
            }

            float voiceTextR = headRight - (headHintW > 0f ? headHintW + 24f : 0f);
            ...
            // ★서천 — 어절 표시가 붙을 자리.
            //   ⚠️ 대사의 <b>자식</b>으로 두면 안 된다. UGUI 는 부모를 먼저 그리므로
            //      강조가 <b>글자를 덮어</b> 흐려진다. 대사와 <b>형제</b>로 두고 맨 앞에 세워
            //      글자 뒤에 깔리게 한다. 대신 굴림을 따라가도록 자리를 함께 옮긴다
            //      (<see cref="ApplyLineScroll"/>).
            pickRoot = MakeRect(lineViewport, "지목표시");
            pickRoot.anchorMin = lineRect.anchorMin;
            pickRoot.anchorMax = lineRect.anchorMax;
            pickRoot.pivot = lineRect.pivot;
            pickRoot.sizeDelta = lineRect.sizeDelta;
            pickRoot.anchoredPosition = lineRect.anchoredPosition;
            pickRoot.SetAsFirstSibling();

            // 「더 있음」 — ★선택지 줄이 들어오면서 대사 <b>아래</b> 틈이 16 밖에 안 남았다(글자가 22).
            //   구분선과 대사 사이(ruleToLine)로 옮긴다. 대사 상자 바로 위라 눈이 따라가기도 쉽고,
            //   ⚠️ 머리 띠로 올리면 안내줄(PC)과 <b>가로세로 모두 겹친다</b> — 실측으로 확인했다.
            if (overflowHint != null)
            {
                if (st.askRows > 0)
                    Place(overflowHint.rectTransform,
                          new Vector2(hw - st.padX - 105f, lineY + st.lineBoxH * 0.5f + st.ruleToLine * 0.5f),
            ...
            // ── ★서천 — 선택지 줄 ──
            //   대사와 입력줄 <b>사이</b>. 자리는 위에서 내려온 대사 밑변에서 lineToAsk 만큼 띄운 곳이다.
            float askTop = lineY - st.lineBoxH * 0.5f - st.lineToAsk;
            if (st.askRows > 0) AskGrid(askTop, geom.w - st.padX * 2f, st);

            // ── 아래에서부터 쌓는다 ──
            //   테두리 → footToEdge → 안내 → inputToFoot → 입력줄
            //   위에서 내려오는 선택지(또는 대사)와 만나는 자리가 lineToInput 이고,
            ...
            // 안내줄 — 머리 띠로 올린 모드에서는 여기서 또 짓지 않는다.
            if (!st.footInNameRow) Footer(st.footSize, footY, st.footH);
            if (hintText != null) hintText.color = pal.dim;
        }

        /// <summary>
        /// ★서천 — 선택지 칸을 깐다. PC 1×4 · VR 2×2 (칸 나눔은 <see cref="SeocheonBarStyle.askCols"/>).
        ///
        /// 단추는 <see cref="MakeButton"/> 이 짓는다 — <b>견우의 그 함수 그대로</b>다.
        /// 그래서 목재 결·한지 글씨·가리키면 금빛으로 물드는 규칙이 다른 단추와 하나도 안 다르다.
        /// 색은 <see cref="InventorySkin.Wood"/> 로 잡았다: 주칠(<see cref="InventorySkin.Vermilion"/>)은
        /// 「묻 기」 하나에만 두어야 <b>보내는 단추</b>가 눈에 띈다.
        ///
        /// ⚠️ 손잡이 종류를 <c>Kind.묻기</c> 로 준다 — 견우 <see cref="DialogueHotspot.Kind"/> 는
        ///    꾸러미 것이라 값을 더할 수 없다(고치면 다음 배포본에 덮인다).
        ///    대신 <see cref="ClickHovered"/> 가 <b>어느 것이 눌렸는지를 먼저</b> 본다.
        /// </summary>
        void AskGrid(float topY, float totalW, SeocheonBarStyle st)
        {
            var pal = IMUNROK.Ui.DialogueUI.Palette();
            int cols = Mathf.Max(1, st.askCols);
            int rows = Mathf.Max(1, st.askRows);
            float cellW = (totalW - st.askGap * (cols - 1)) / cols;

            for (int i = 0; i < askOptSpots.Length; i++)
            {
                int c = i % cols, r = i / cols;
                if (r >= rows) break;
                float x = -totalW * 0.5f + cellW * 0.5f + c * (cellW + st.askGap);
                float y = topY - st.askH * 0.5f - r * (st.askH + st.askGap);

                TextMeshProUGUI lab;
                askOptSpots[i] = MakeButton(root, "선택지" + (i + 1), new Vector2(x, y),
                                            new Vector2(cellW, st.askH),
                                            DialogueHotspot.Kind.묻기, InventorySkin.Wood,
                                            st.inputSize, out lab);
                askOptLabels[i] = lab;
                lab.text = string.Empty;
                // 긴 물음이 칸을 넘치지 않게 — 넘치면 줄을 버리는 것이 아니라 <b>줄여서</b> 담는다.
                lab.enableAutoSizing = true;
                lab.fontSizeMin = st.inputSize - 8;
                lab.fontSizeMax = st.inputSize;
                askOptSpots[i].gameObject.SetActive(false);   // 물어 보기 전에는 비어 있다
            }
        }
